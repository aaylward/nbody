...
## 2026-05-01 - [Avoid Array Allocations in High-Frequency WebGPU Loops]
**Learning:** In high-frequency WebGPU loops (like physics integration or rendering), repeatedly allocating `new Float32Array` or `new ArrayBuffer` for uniform updates causes significant memory churn and micro-stutters due to garbage collection overhead.
**Action:** Always pre-allocate uniform buffer views (e.g. `Float32Array` over a shared `ArrayBuffer`) at the class level and update them in-place using array indexing before calling `device.queue.writeBuffer`.
