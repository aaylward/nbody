...
## 2024-05-18 - Hoisting TypedArray Allocations in Hot Loops
**Learning:** Instantiating small, temporary typed arrays (e.g., `new Float32Array([alpha, 0, 0, 0])`) or `new ArrayBuffer(...)` directly inside 60FPS render loops or high-frequency physics ticks generates thousands of allocations per second, resulting in significant GC pressure and micro-stutters, especially in WebGPU implementations where uniforms are continuously updated.
**Action:** When updating WebGPU uniform buffers (e.g., via `device.queue.writeBuffer`) in a hot loop, always pre-allocate the required memory object (`ArrayBuffer`, `Float32Array`, `Uint32Array`) as class instance variables during initialization. Mutate the specific indices of these pre-allocated buffers inside the loop to achieve zero-allocation performance.
