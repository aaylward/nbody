...
## 2024-03-08 - WebGPU BindGroup Instantiation in Hot Loops
**Learning:** Calling `device.createBindGroup()` inside a per-frame physics loop introduces massive pipeline validation and object allocation overhead in WebGPU, which can bottleneck double-buffered simulations.
**Action:** Always pre-allocate arrays of `GPUBindGroup` objects during initialization and swap integer indices (`this.currentBufferIndex = 1 - this.currentBufferIndex`) in the render loop to completely avoid runtime allocations.
