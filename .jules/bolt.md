...
## 2025-05-27 - [Optimized Statistics Calculations]
**Learning:** Calculating averages using `Array.prototype.reduce` and calculating percentiles using array sorting `[...arr].sort()` scales poorly (O(N log N) for sort) and introduces high constant overhead in V8 when computing statistics frequently in hot paths.
**Action:** Replace `reduce` with a simple `for` loop for averages, and replace full sorting with the QuickSelect algorithm (O(N) on average) for percentiles to achieve substantial speedups (30x-40x) when fetching performance statistics during simulation.
