...
## 2025-05-19 - [Optimized Realtime GPU Hot Loop Allocation]
**Learning:** In hot game/simulation loops (e.g. `useFrame` or `while (running)`), repeatedly allocating short-lived typed arrays (like `new Float32Array([alpha, 0, 0, 0])` or `new ArrayBuffer(16)`) triggers the V8 garbage collector frequently, leading to frame rate micro-stutters and increased CPU-GPU overhead.
**Action:** When updating WebGPU uniforms in a render or physics loop, allocate the underlying `ArrayBuffer` and typed array views (e.g. `Float32Array`) once as class fields and update their elements in-place. Then pass the cached array to `device.queue.writeBuffer`.
