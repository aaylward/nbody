...
## 2025-06-14 - Replace O(N log N) Native Sort with QuickSelect and native loop for reduce in performance monitoring
**Learning:** Calculating statistical percentiles in high-frequency rendering using `[...arr].sort()` degrades performance scaling to O(N log N). Furthermore, calculating averages using `Array.prototype.reduce` incurs significant functional overhead in hot V8 loops compared to native `for` loop iterations.
**Action:** When computing percentiles, implement a QuickSelect algorithm with a random pivot and 3-way partition (Dutch National Flag) to reduce time complexity to O(N) on average while preventing O(N²) worst-case performance on arrays with many identical elements. Use simple `for` loops instead of `Array.prototype.reduce` for calculating averages in hot loops.
