...
## 2025-05-19 - [Optimized Realtime Tracking Arrays]
**Learning:** In continuous tracking systems (like performance monitors or frame counters), using standard arrays with `.push()` and `.shift()` causes continuous memory allocation and garbage collection overhead in hot loops (e.g., `useFrame` or physics loops).
**Action:** Use fixed-length `Float64Array` (or appropriate TypedArray) circular buffers with indexed circular overwriting to eliminate array allocations and GC overhead for continuous high-frequency statistics tracking.
