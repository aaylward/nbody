...
## 2025-05-19 - [Optimized WebGPU Uniform Buffer Updates in Hot Loops]
**Learning:** In WebGPU hot loops (e.g. `requestAnimationFrame` or high-frequency physics tick), recreating `new ArrayBuffer`, `new Float32Array`, or `new Uint32Array` on every frame for uniform buffer updates (like `device.queue.writeBuffer(uniforms, 0, new Float32Array([...]))`) causes rapid memory churn. This results in heavy garbage collection pressure, leading to frame drops and micro-stutters.
**Action:** When updating WebGPU uniform buffers every frame, pre-allocate the host-side `ArrayBuffer` and its typed array views as class properties. In the hot loop, mutate these views in-place and pass the pre-allocated buffer to `device.queue.writeBuffer()`.
