...
## 2023-10-27 - [Optimized GPU Uniform Buffer Allocations]
**Learning:** In real-time WebGPU render and physics loops (e.g., `RealtimeSimulation` and `RealtimeSimulationGPUBarnesHut`), allocating new `Float32Array` or `ArrayBuffer` objects inside the loop every frame creates unnecessary memory churn and triggers garbage collection, leading to micro-stutters.
**Action:** Always pre-allocate fixed-size array buffers and TypedArray views as class properties during initialization. Update their contents in-place and pass the reused buffer to `device.queue.writeBuffer()` to eliminate per-frame allocations.
