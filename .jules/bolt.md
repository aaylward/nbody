...
## 2024-05-20 - WebGPU Shader Acceleration Optimization
**Learning:** In WGSL compute shaders for N-body simulations, calculating forces (`f = G * m1 * m2 / r^2`) and later dividing by mass (`a = f / m`) wastes compute cycles by multiplying by mass and then dividing by mass later. Additionally, WGSL provides a fast intrinsic `inverseSqrt()` which is significantly faster than `1.0 / sqrt()`.
**Action:** Always compute acceleration directly (`a = G * m2 / r^2`) and store it in an acceleration buffer, eliminating the `O(N^2)` multiplications and `O(N)` divisions. Use WGSL's `inverseSqrt(r2)` instead of `1.0 / sqrt(r2)` for optimal distance calculation performance.
