...
function computeForcesCPU(particles: Float32Array | Float64Array, forces: Float32Array | Float64Array, numParticles: number) {
  ...
  for (let i = 0; i < numParticles; i++) {
    ...
    for (let j = i + 1; j < numParticles; j++) {
      ...
      // Note: In V8, computing division directly is ~10-15% faster than calculating
      // inverse square root and multiplying (invR * invR * invR) as it avoids extra
      // multiplications and variable assignments inside the O(N^2) loop.
