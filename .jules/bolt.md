...
## 2023-11-20 - PerformanceMonitor: Fixed Length Circular Buffers
**Learning:** Using `Array.push()` and `Array.shift()` to maintain a fixed-size queue for performance monitoring causes continuous GC overhead and slows down data insertion. Recomputing averages via `reduce` and percentiles via `[...arr].sort()` over JS arrays has unnecessary overhead compared to loop implementations and TypedArrays.
**Action:** In continuous tracking systems (like performance monitors or frame counters), avoid using standard arrays with `.push()` and `.shift()`. Instead, use fixed-length `Float64Array` circular buffers with indexed circular overwriting to eliminate array allocations and garbage collection overhead.
