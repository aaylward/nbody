...
## 2025-05-20 - [Optimized Memory Allocations in Array Utility Iterations]
**Learning:** In utility functions processing typed arrays (like `toParticleObjects`, `extractPositions`, `extractVelocities`), allocating new objects or arrays inside loop iteration bodies causes massive amounts of GC pressure in tight inner loops or high frequency render paths. By supporting optional `out` parameters to reuse memory, the duration and latency of data conversion steps drop drastically (e.g. `toParticleObjects` drops from 3.362s to 241ms in V8 over 100k particles). Furthermore, replacing post-increments (`idx++`) with explicit offsets (`[idx]`, `[idx+1]`, `idx+=3`) yields measurable improvements in extraction speeds.
**Action:** Always provide and utilize optional `out` arguments for buffer/array methods running in tight V8 loops to re-use memory and avoid per-iteration allocations.
