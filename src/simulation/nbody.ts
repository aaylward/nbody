...
function interpolateSnapshots(
  ...
  for (let i = 0; i < snapshots.length - 1; i++) {
    ...
    // Optimization: Loop over frames first, then properties. Even though hoisting math outside the frame loop
    // saves some arithmetic operations, V8's JIT compiler optimizes the sequential memory writes much better
    // when iterating linearly over the target arrays (frames) rather than jumping between them.
    for (let j = 0; j < numFrames; j++) {
      const t = ts[j];
      const interpolated = frames[j];
      for (let offset = 0; offset < numFloats; offset += FLOATS_PER_PARTICLE) {
        interpolated[offset + OFFSET_X] = snap1[offset + OFFSET_X] + (snap2[offset + OFFSET_X] - snap1[offset + OFFSET_X]) * t;
        interpolated[offset + OFFSET_Y] = snap1[offset + OFFSET_Y] + (snap2[offset + OFFSET_Y] - snap1[offset + OFFSET_Y]) * t;
        interpolated[offset + OFFSET_Z] = snap1[offset + OFFSET_Z] + (snap2[offset + OFFSET_Z] - snap1[offset + OFFSET_Z]) * t;
        interpolated[offset + OFFSET_VX] = snap1[offset + OFFSET_VX] + (snap2[offset + OFFSET_VX] - snap1[offset + OFFSET_VX]) * t;
        interpolated[offset + OFFSET_VY] = snap1[offset + OFFSET_VY] + (snap2[offset + OFFSET_VY] - snap1[offset + OFFSET_VY]) * t;
        interpolated[offset + OFFSET_VZ] = snap1[offset + OFFSET_VZ] + (snap2[offset + OFFSET_VZ] - snap1[offset + OFFSET_VZ]) * t;
      }
