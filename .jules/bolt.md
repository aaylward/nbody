...
## 2025-05-19 - [Optimized Realtime GPU Render Sync]
...
## 2024-05-19 - [Optimized WebGPU API Call Overhead]
**Learning:** In high-frequency WebGPU rendering or physics loops (e.g. `requestAnimationFrame`), recreating stateful API objects like `GPUBindGroup` or calling `createBuffer` per frame introduces severe CPU overhead, driver validation latency, and micro-stutters.
**Action:** When handling dynamic state transitions like double-buffering in WebGPU hot loops, do not recreate `GPUBindGroup` objects on the fly. Instead, pre-allocate arrays of bind groups (e.g., one for each buffer state) during initialization and toggle an integer index to swap them, completely avoiding per-frame reconstruction overhead.

## 2024-05-19 - [Eliminated GC Churn in Uniform Buffer Updates]
**Learning:** Instantiating new TypedArrays (like `new Float32Array([alpha, 0, 0, 0])`) or ArrayBuffers per-frame just to feed `device.queue.writeBuffer()` causes significant Javascript heap allocations and garbage collection pauses in WebGPU hot loops.
**Action:** Hoist the instantiation of short-lived buffers (e.g. uniform data) into class properties. Update their elements directly in place per-frame and pass the reused buffer to the WebGPU write queue to eliminate unnecessary GC churn.
