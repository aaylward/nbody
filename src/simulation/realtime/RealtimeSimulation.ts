...
export class RealtimeNBodySimulation {
  ...
  private setupGPU(): void {
    ...
    // Create bind groups (pre-allocated to avoid recreation per frame)
  ...
  private setupBindGroups(): void {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      this.interpolateBindGroups.push(
        this.device.createBindGroup({
  ...
  private async physicsLoop(): Promise<void> {
    while (this.running) {
      ...
      // Swap GPU buffers by updating index (double buffering)
