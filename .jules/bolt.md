...
\n## 2026-06-03 - [Pre-allocate GPUBindGroups in WebGPU Hot Loop]\n**Learning:** Recreating `GPUBindGroup` objects on every frame inside a WebGPU rendering or physics hot loop (e.g., to implement double buffering) introduces substantial CPU overhead and stalls the render pipeline. WebGPU is designed so that bind groups are created once during initialization.\n**Action:** When handling dynamic state transitions like double buffering, pre-allocate arrays of bind groups (e.g., one for each buffer state) during setup. Then, during the hot loop, simply toggle an integer index to swap them, completely bypassing per-frame reconstruction.
