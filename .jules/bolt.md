...
## 2025-06-25 - [Pre-allocated Bind Groups for WebGPU]
**Learning:** Recreating WebGPU `GPUBindGroup`s dynamically inside the inner frame-loop of double-buffered applications forces expensive CPU-side re-validation and allocations, degrading JS performance.
**Action:** When handling dynamic state transitions like double-buffering in WebGPU loops, avoid recreating `GPUBindGroup` objects on the fly. Pre-allocate arrays of bind groups (e.g., one for each buffer state) during initialization and toggle an integer index to swap them, completely bypassing instantiation overhead per frame.
