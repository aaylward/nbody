...
## 2026-06-27 - [Optimized WebGPU Bind Group Instantiation]
**Learning:** In WebGPU physics hot loops, recreating `GPUBindGroup` objects on every frame (e.g. during double-buffering) introduces significant CPU overhead and stalling. Although the underlying `GPUBuffer` objects exist, binding new views to them in tight loops is slow.
**Action:** When implementing double-buffering or dynamic state transitions in WebGPU, pre-allocate arrays of `GPUBindGroup` objects (one for each buffer state) during initialization. Toggle between states using an integer index to avoid per-frame instantiation entirely.
