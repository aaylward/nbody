...
## 2025-05-19 - [Optimized Realtime Performance Monitor Circular Buffer]
**Learning:** In continuous performance tracking monitors, using standard JavaScript arrays with `.push()` and `.shift()` creates ongoing memory allocations and triggers garbage collection. For systems tracking frequent high-frequency loops (like FPS monitors tracking every frame), this causes unnecessary micro-stutters and overhead.
**Action:** Replace standard arrays for continuous fixed-length time series with `Float64Array` circular buffers (using an explicit `index` and `count` variable). For related statistical calculations, use manual `for` loops or `.slice().sort()` rather than allocating generic copies (`[...arr]`).
