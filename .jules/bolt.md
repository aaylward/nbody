...
## 2024-05-23 - [Optimized Memory Allocations in Array Generation]
**Learning:** In performance-critical functions that process `TypedArrays` (like `extractPositions`, `extractVelocities`, and `toParticleObjects`), sequentially assigning elements with a post-increment (`arr[idx++]`) introduces measurable iteration overhead in V8 compared to explicit index offsets with manual increments (`arr[idx] = ...; arr[idx + 1] = ...; idx += N`).
**Action:** When extracting interleaved properties into compact arrays, use direct property assignment on known fixed offset layouts and avoid `++` expressions inside the assignment operator. Additionally, adding an optional `out` parameter to these functions to reuse memory buffers entirely eliminates large allocations and significantly decreases latency and GC pauses.
