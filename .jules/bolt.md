...
## 2025-05-19 - [Optimized PerformanceMonitor Array Churn]
**Learning:** In continuous tracking systems (like performance monitors or frame counters), using standard arrays with `.push()` and `.shift()` creates continuous memory allocations and reallocations on every frame. Furthermore, using `[...arr].sort()` for percentiles and `arr.reduce()` for averages creates more intermediate arrays and functions. For small bounded arrays (e.g., 60 samples), this creates severe garbage collection pressure in a hot loop.
**Action:** Replace `number[]` with fixed-size `Float64Array` circular buffers with indexed circular overwriting to eliminate array allocations. Use manual `for` loops for average calculations and `TypedArray.prototype.sort()` for percentile calculations to eliminate garbage collection overhead.
