...
## 2026-05-19 - [Optimized Buffer Allocations in Hot WebGPU Loops]
**Learning:** Re-allocating `ArrayBuffer` and typed array views (like `new Float32Array([...])`) on every frame to write to WebGPU uniform buffers (`device.queue.writeBuffer`) causes significant JavaScript garbage collection overhead and potential frame stutters in a hot physics or render loop.
**Action:** Hoist these temporary buffer arrays and their views to class-level properties. In the hot loop, mutate their contents in-place and pass the same reused buffer reference to `writeBuffer`.
