...
## 2024-05-18 - Eliminating Hot Loop Object and Array Buffer Allocations
**Learning:** Instantiating `new Float32Array(...)` or `new ArrayBuffer(...)` directly inside WebGPU dispatch loops (`physicsLoop`, `getRenderPositionBuffer`) creates immense garbage collection pressure, leading to micro-stutters.
**Action:** When working with WebGPU uniforms that update per-frame, always cache the TypedArray and ArrayBuffer at the class level during initialization and reuse them by mutating indices directly (e.g., `this.forcesUniformsU32[0] = this.numParticles; this.device.queue.writeBuffer(..., this.forcesUniformsHostBuffer)`).
