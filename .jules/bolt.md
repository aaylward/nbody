...
## 2025-05-19 - [Optimized WebGPU Object Instantiation in Hot Loops]
**Learning:** In double-buffered WebGPU simulation loops, calling `device.createBindGroup()` on every frame to swap storage buffers introduces significant CPU instantiation overhead and jank. Similarly, repeatedly allocating `ArrayBuffer` or `Float32Array` objects to upload uniform values (like alpha or deltaT) causes heavy garbage collection pressure.
**Action:** When handling dynamic state transitions like double-buffering, pre-allocate arrays of `GPUBindGroup` objects (one for each buffer state) during setup. In the render loop, toggle an integer index to swap them. Always hoist fixed-size ArrayBuffer allocations for uniforms out of hot loops into class fields.
