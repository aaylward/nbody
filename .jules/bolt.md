...
## 2025-06-26 - [Optimized Performance Monitor Arrays]
**Learning:** In continuous tracking systems (like performance monitors or frame counters), avoid using standard arrays with `.push()` and `.shift()`. Instead, use fixed-length `Float64Array` circular buffers with indexed circular overwriting to eliminate array allocations and garbage collection overhead. Calculating statistical averages using `Array.prototype.reduce` and percentiles using `[...arr].sort()` scales poorly in V8 hot paths. Use a `for` loop for averages and QuickSelect for percentiles to achieve substantial speedups.
**Action:** Replace standard arrays that update frequently with fixed-size typed arrays, handle wrapping around using circular indices, and replace array functional operations (`.reduce()`, `.sort()`) with imperative `for` loops or QuickSelect for finding percentiles without sorting the entire array.
