...
function computeForcesCPU(particles: Float32Array | Float64Array, forces: Float32Array | Float64Array, numParticles: number) {
  ...
  for (let i = 0; i < numParticles; i++) {
    ...
    for (let j = i + 1; j < numParticles; j++) {
      ...
      // Optimization: Although division is generally slower than multiplication on CPUs, computing
      // 1 / (r2 * sqrt(r2)) in modern V8 engines is ~10-15% faster than calculating invR and multiplying
      // invR * invR * invR, because it avoids multiple multiplication instructions.
