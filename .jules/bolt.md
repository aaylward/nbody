...
## 2025-05-24 - [Optimized Uniform Buffer Updates]
**Learning:** In WebGPU hot loops (like a 60 FPS render loop or high-frequency physics step), creating a new `ArrayBuffer` or `Float32Array` on every frame just to pass to `device.queue.writeBuffer` introduces measurable GC churn and memory allocation overhead. Benchmarks show that instantiating a new `Float32Array` on every frame is orders of magnitude slower than reusing an existing pre-allocated array.
**Action:** When updating WebGPU uniform buffers dynamically, pre-allocate the required `ArrayBuffer` or TypedArray as a class property and update its contents (e.g., `this.myUniformArray[0] = newValue`) before passing the reusable buffer to `writeBuffer`.
