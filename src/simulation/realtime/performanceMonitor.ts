...
export class PerformanceMonitor {
  ...
  private percentile(arr: Float64Array, count: number, p: number): number {
    if (count === 0) return 0;
    // For small fixed-size arrays (e.g. 60 elements), TypedArray.prototype.sort is very fast.
    // Use a copy to avoid mutating the original buffer
    const sorted = new Float64Array(arr.buffer, 0, count).slice().sort();
