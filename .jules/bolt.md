...
## 2025-06-25 - [Optimized Rolling Statistics Calculation]
**Learning:** In continuous tracking systems (like performance monitors calculating rolling averages and percentiles), standard JavaScript arrays with `.push()` and `.shift()` create O(N) allocation and garbage collection overhead. Also, native `.sort()` is O(N log N) which is slow in hot paths.
**Action:** Use fixed-length `Float64Array` circular buffers with manual `index` and `count` tracking for zero-allocation stats collection. For calculating percentiles, use an in-place QuickSelect algorithm with a random pivot and Dutch National Flag 3-way partition to achieve O(N) time complexity even when many identical frame times exist.
