...
## 2025-05-19 - [Optimized WebGPU Buffers and Bind Groups in Hot Loops]
**Learning:** Creating WebGPU buffers (`device.createBuffer`) or recreating bind groups (`device.createBindGroup`) within hot inner loops (like `useFrame` or `physicsLoop`) triggers severe performance bottlenecks, memory allocations, and garbage collection pauses. Real-time CPU-GPU rendering pipelines should never reconstruct WebGPU primitives per frame.
**Action:** Always pre-allocate all required WebGPU buffers and `GPUBindGroup`s during simulation initialization. If dynamic state transitions are necessary (e.g., swapping particle buffers for double buffering), maintain a pre-allocated array of bind groups and toggle an integer index (`0` or `1`) instead of allocating new bind groups on the fly. Update uniforms using `queue.writeBuffer` with a single, hoisted, pre-allocated `TypedArray` wrapper (like `Float32Array(4)`) to completely eliminate GC pressure.
