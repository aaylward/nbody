...
## 2025-05-20 - [Optimized Realtime GPU Double Buffering]
**Learning:** In WebGPU hot loops (e.g., `physicsLoop`), recreating `GPUBindGroup` objects every frame to swap between double-buffered data introduces significant memory churn, instantiation overhead, and garbage collection pauses.
**Action:** Pre-allocate bind groups during setup into static arrays indexed by the buffer state (`0` or `1`). During the physics loop, simply toggle an integer index (`this.currentBufferIndex = 1 - this.currentBufferIndex`) and pass the pre-allocated bind group to the command encoder.
