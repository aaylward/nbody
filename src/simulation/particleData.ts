...
export function toParticleObjects(
  ...
  // Sync length if an existing array is passed but sized incorrectly
  if (out && particles.length !== numParticles) {
    particles.length = numParticles;
  }

  // Optimization: Inline property reading, reuse existing objects to avoid
  // garbage collection overhead, and increment offsets manually (~2x faster)
  let pIdx = 0;
...
export function extractPositions(data: Float32Array, out?: Float32Array): Float32Array {
  ...
  // Optimization: Iterate by offset directly, use explicit offset index reads,
  // and manually increment pointer.
  let pIdx = 0;
  for (let offset = 0; offset < numFloats; offset += FLOATS_PER_PARTICLE) {
