...
## 2025-05-19 - [Optimized Performance Monitoring Circular Buffer & Percentile Calculation]
**Learning:** In `PerformanceMonitor`, continuous array manipulations using `[].push()` and `[].shift()` cause memory churn and trigger unnecessary garbage collection for high-frequency (60fps) stat logging. Similarly, `reduce` for averaging and `sort` for percentiles scale poorly in V8 hot paths. Replacing standard arrays with pre-allocated `Float64Array` circular buffers alongside `for` loops for averaging and a QuickSelect algorithm for percentiles speeds up performance stat tracking by over 3.5x.
**Action:** In continuous tracking systems, avoid using standard arrays with `.push()` and `.shift()`. Instead, use fixed-length `Float64Array` circular buffers with indexed circular overwriting to eliminate array allocations and garbage collection overhead. Calculate statistical averages using a `for` loop and percentiles using QuickSelect rather than `.sort()`.
