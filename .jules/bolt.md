...
## 2025-05-19 - [Optimized Memory Pattern for TypedArray Interpolation]
**Learning:** When interpolating `Float32Array` values across frames (e.g. `[...frames].forEach(f => f[i] = ...)`) in V8, iterating linearly across the destination array by looping over *frames first* then *offsets* is significantly faster than looping over *offsets first* then *frames*. Even though hoisting the mathematical differences (`dx = s2 - s1`) to the outer loop theoretically reduces arithmetic operations, V8's JIT compiler penalizes non-sequential memory jumps more heavily.
**Action:** When filling arrays representing frames, always make the frame the *outer loop* and sequentially iterate through the memory offsets inside the inner loop to ensure high cache locality and fast JIT optimizations.
