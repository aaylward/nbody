...
## 2025-05-24 - [Optimized WebGPU Bind Group Instantiation]
**Learning:** In WebGPU simulation hot loops that use double-buffering (like Leapfrog integration), calling `device.createBindGroup()` on every frame for each pipeline stage leads to massive memory churn and garbage collection pressure, as well as driver-level overhead.
**Action:** When handling dynamic state transitions like double-buffering in WebGPU, pre-allocate arrays of `GPUBindGroup` objects during initialization (e.g. `this.forceBindGroups = [0, 1].map(...)`). Toggle an integer index (`currentBufferIndex = 1 - currentBufferIndex`) every frame to swap between them, completely eliminating per-frame instantiation overhead.
