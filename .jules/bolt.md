...
## 2025-05-19 - [Optimized Circular Buffers and Array Sorting]
**Learning:** Using standard arrays with `.push()` and `.shift()` for continuous telemetry tracking (like FPS or timings) causes constant allocations and GC pauses. Pre-allocated `Float64Array` ring buffers eliminate this overhead. Additionally, unlike `Array.prototype.sort()`, `TypedArray.prototype.sort()` sorts numerically by default, making it safe and concise to use without a custom comparator for small arrays (<= 60 items).
**Action:** Use pre-allocated `Float64Array` buffers for high-frequency tracking arrays instead of `push`/`shift`. For small fixed-size arrays, prefer `TypedArray.prototype.sort()` over custom algorithms like QuickSelect to maintain readability while still avoiding JS Array quirks.
