...
## 2025-05-19 - [Pre-allocate Uniform Buffers in Hot Loops]
**Learning:** In high-frequency WebGPU physics or render loops (e.g., `physicsLoop` in Barnes-Hut simulation), allocating new `ArrayBuffer` and TypedArray views per frame causes memory churn and garbage collection pressure.
**Action:** Small, fixed-size buffer allocations (e.g., for uniforms) should be hoisted outside the loop as class fields, reusing the buffer with `device.queue.writeBuffer(..., 0, reusableArrayBuffer)`.
