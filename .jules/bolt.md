...
## 2025-05-19 - [Optimized WebGPU BindGroup Allocation]
**Learning:** In WebGPU double-buffering architectures, dynamically recreating `GPUBindGroup` objects on every frame (to point to alternating buffers) introduces unnecessary memory churn and WebGPU API overhead in the hot render/compute loop.
**Action:** Pre-allocate arrays of bind groups (e.g., `bindGroups[0]` and `bindGroups[1]`) during initialization for each double-buffer state, and toggle an integer index (`currentBufferIndex = 1 - currentBufferIndex`) to swap them seamlessly during the simulation loop without per-frame allocations.
