...
export class PerformanceMonitor {
  ...
  private percentile(arr: Float64Array, count: number, p: number): number {
    ...
    // Create a temporary array of just the valid elements
    const copy = new Float64Array(count);
    for (let i = 0; i < count; i++) {
      copy[i] = arr[i];
    }

    // For a small array (<= 60 elements), built-in sort is fast enough and much more readable
    copy.sort();

    const index = Math.floor(count * p);
    // Ensure we don't go out of bounds
    return copy[Math.max(0, Math.min(index, count - 1))];
  }
}
