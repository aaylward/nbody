...
export class RealtimeNBodySimulationGPUBarnesHut {
  ...
  // Pre-allocated buffers to prevent per-frame memory churn
  ...
  constructor(options: RealtimeSimulationGPUBarnesHutOptions) {
    ...
    // Initialize reusable uniform buffers
  ...
  private async physicsLoop(): Promise<void> {
    ...
    while (this.running) {
      ...
      // We reuse the pre-allocated buffer to avoid new ArrayBuffer allocations per frame.
