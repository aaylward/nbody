...
## 2025-05-20 - [Optimized Realtime WebGPU Bind Group Allocation]
**Learning:** In WebGPU hot loops handling dynamic state transitions like double-buffering, recreating `GPUBindGroup` objects on the fly each frame causes significant memory churn and per-frame overhead.
**Action:** When handling double-buffering or dynamic state, pre-allocate arrays of bind groups (e.g., `[GPUBindGroup, GPUBindGroup]`) for each state during initialization and use an integer index (`currentBufferIndex = 0`) to swap them in the render/physics loop, eliminating per-frame instantiation entirely.
