...
## 2025-06-05 - [Optimized WebGPU Double Buffering]
**Learning:** When handling dynamic state transitions like double-buffering in WebGPU hot loops, do not recreate `GPUBindGroup` objects on the fly by calling `createBindGroup` in a loop or calling a function that creates them. Instead, pre-allocate arrays of bind groups (e.g., one for each buffer state) during initialization and toggle an integer index to swap them. This avoids widespread state desynchronization across the class and avoids per-frame object instantiation overhead.
**Action:** Use an array of `GPUBindGroup[]` and an index (e.g. `currentBufferIndex`) to swap states rather than swapping individual references and rebuilding bind groups.
