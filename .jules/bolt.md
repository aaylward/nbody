...
## 2025-03-24 - [Optimized TypedArray Iteration using Memory Offsets]
**Learning:** In tight conversion loops mapping object arrays to TypedArrays (like \`toParticleObjects\` and \`fromParticleObjects\`), or extracting structural subcomponents (like \`extractPositions\` or \`extractVelocities\`), using per-particle loops and multiplying by \`FLOATS_PER_PARTICLE\` inside the loop induces noticeable overhead.
**Action:** Replace multiplication loops (\`for i.. \` with \`i * offset\`) with direct memory offset iteration (\`for offset.. \` and \`offset += offset\`). Combine this with inline getters/setters and separate accumulator indices to maximize performance by avoiding redundant calculations and O(N) calls to outer functions.
