...
## 2025-06-25 - [Optimized WebGPU Double Buffering]
**Learning:** Re-creating `GPUBindGroup` objects inside the main simulation loop on every frame to achieve double buffering introduces considerable garbage collection pressure and object instantiation overhead, stalling high-frequency WebGPU event pipelines.
**Action:** When implementing double buffering (or ping-ponging) in WebGPU hot loops, create pre-allocated arrays of bind groups mapping to each buffer state during initialization. Instead of recreating bind groups per frame, simply toggle an integer index (e.g., `currentBufferIndex = 1 - currentBufferIndex`) to swap them instantly.
