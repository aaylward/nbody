...
export class PerformanceMonitor {
  ...
  // Use fixed-length Float64Array circular buffers to prevent array allocations and GC overhead
  private physicsTimings: Float64Array = new Float64Array(this.maxSamples);
  private renderTimings: Float64Array = new Float64Array(this.maxSamples);
  ...
  private percentile(arr: Float64Array, count: number, p: number): number {
    if (count === 0) return 0;
    // Slice up to the current count and sort numerically in-place
    // This avoids QuickSelect overhead for small arrays while maintaining immutability of the original buffer
