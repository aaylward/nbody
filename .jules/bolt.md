...
## 2025-05-19 - [Optimized WebGPU Bind Group Instantiation]
**Learning:** In WebGPU hot loops (like double-buffering particle physics frames), calling `device.createBindGroup()` on the fly every frame to update buffer bindings introduces massive instantiation overhead and micro-stutters from memory churn.
**Action:** When handling dynamic state transitions like double-buffering in WebGPU, pre-allocate arrays of bind groups (e.g., one for each buffer state) during initialization. Toggle an integer index during the render/compute loop to swap the active bind group, completely avoiding per-frame reconstruction overhead.
