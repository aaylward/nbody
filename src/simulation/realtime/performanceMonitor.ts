...
export class PerformanceMonitor {
  ...
  private percentile(arr: Float64Array, count: number, p: number): number {
    if (count === 0) return 0;
    // For small fixed-size arrays (e.g., 60 elements in a performance monitor),
    // prefer TypedArray.prototype.sort() to maintain readability.
    // .slice() creates a shallow copy, so it doesn't modify the original buffer.
