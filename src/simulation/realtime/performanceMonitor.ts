...
export class PerformanceMonitor {
  // Optimization: Use fixed-length Float64Array circular buffers instead of Arrays with .push()/.shift()
  // to eliminate memory allocations and garbage collection overhead in hot render/physics loops.
  ...
  private percentile(arr: Float64Array, count: number, p: number): number {
    if (count === 0) return 0;
    // Optimization: TypedArray.slice() copies memory fast, and TypedArray.sort() works in-place
    // and sorts numerically by default, avoiding the overhead of creating and sorting standard Arrays.
