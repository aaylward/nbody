...
## 2025-05-19 - [Avoid WebGPU BindGroup Recreation in Hot Loops]
**Learning:** Recreating `GPUBindGroup` objects inside high-frequency WebGPU physics or render loops (e.g. for double-buffering state swaps) causes significant per-frame overhead and memory churn.
**Action:** When handling dynamic state transitions like double-buffering, pre-allocate arrays of bind groups (one for each buffer state) during initialization. Then, toggle an integer index to swap them within the loop, completely avoiding the overhead of `device.createBindGroup` per frame.
