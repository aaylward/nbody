...
## 2025-05-20 - [Optimized Realtime Performance Monitor]
**Learning:** Using standard `Array` with `.push()` and `.shift()` for continuous telemetry tracking (e.g., rendering and physics FPS loops) introduces thousands of array allocations and subsequent garbage collection passes. This constant GC overhead causes frame drops and micro-stutters during realtime visualization.
**Action:** When tracking short history windows in hot loops (like FPS counters), replace dynamic arrays with a pre-allocated fixed-size `Float64Array`. Use modulo arithmetic (`index = (index + 1) % maxSamples`) for a circular buffer implementation to entirely eliminate memory churn and prevent GC spikes.
