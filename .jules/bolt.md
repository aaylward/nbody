...
## $(date +%Y-%m-%d) - [Optimized WebGPU Bind Group Allocation]
**Learning:** In high-frequency render/physics loops (like `RealtimeSimulation.ts`), unconditionally recreating WebGPU objects like \`GPUBindGroup\` on every frame using \`device.createBindGroup()\` introduces significant overhead, memory churn, and potential frame drops/stutters due to garbage collection pressure.
**Action:** When handling dynamic state transitions that require changing bindings (such as swapping double-buffered \`GPUBuffer\`s), completely avoid per-frame reconstruction. Instead, pre-allocate arrays of bind groups for all necessary states during initialization and simply toggle an integer index during the loop to swap them.
