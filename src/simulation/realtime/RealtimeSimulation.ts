...
export class RealtimeNBodySimulation {
  ...
  // Optimization: Pre-allocate GPUBindGroup objects for both states of the double buffer
  // to eliminate per-frame WebGPU API allocation overhead and memory churn.
  private initBindGroups(): void {
    for (let i = 0; i < 2; i++) {
      const currentBuffer = this.particleBuffers[i];
      const nextBuffer = this.particleBuffers[1 - i];
