...
## 2025-05-19 - [Optimized Uniform Buffer Updates]
**Learning:** Calling `device.queue.writeBuffer(buffer, 0, new Float32Array([...]))` or `new ArrayBuffer(...)` on every frame in a hot WebGPU render or simulation loop creates heavy garbage collection pressure. This leads to micro-stutters and dropped frames in realtime rendering as V8 is constantly allocating and freeing small uniform arrays.
**Action:** When updating WebGPU uniform buffers frequently, pre-allocate the backing `ArrayBuffer` and its typed views (`Float32Array`, `Uint32Array`) once as class properties. On every frame, simply update the typed view indices in place and pass the same pre-allocated `ArrayBuffer` or `TypedArray` directly to `writeBuffer`.
