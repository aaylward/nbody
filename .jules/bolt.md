...
## 2026-06-16 - [Optimized PerformanceMonitor Allocations and Computations]
**Learning:** For continuous tracking arrays (like fps monitoring), using standard arrays and `.push()` / `.shift()` adds garbage collection overhead and memory churn. Furthermore, `Array.reduce` and `.sort()` scale poorly with repeated fast invocation (like a render or physics loop).
**Action:** Replace dynamically shifted arrays with fixed-length `Float64Array` circular buffers. Use indexed circular overwriting to eliminate array allocations entirely. For statistical computation on the buffer, use explicitly looped averages and a custom `quickSelect` algorithm for percentiles to avoid allocating entirely new sorted arrays.
