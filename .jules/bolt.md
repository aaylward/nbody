...
## 2025-05-19 - [Optimized WebGPU BindGroup Instantiation]
**Learning:** Recreating WebGPU `GPUBindGroup` objects in high-frequency physics or rendering loops (e.g., when swapping double-buffered arrays) forces the JavaScript engine to allocate garbage and the browser graphics driver to perform expensive validation per frame. This results in severe CPU overhead and micro-stutters.
**Action:** When handling dynamic state transitions like double-buffering in WebGPU hot loops, do not recreate `GPUBindGroup` objects on the fly. Pre-allocate arrays of bind groups (e.g., one for each buffer state) during initialization and toggle an integer index to swap them.
